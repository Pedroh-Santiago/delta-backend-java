...
class ListCustomerUseCaseTests {
    ...
    void mapsContentAndReturnsPaginationMetadata() {
        ...
        when(customerRepository.findPage(eq(tenantId), any(), any(), any(), any()))
        ...
        assertEquals("+5511999998888", r.items().get(0).phoneNumber());
        assertEquals("São Paulo", r.items().get(0).city());
        assertEquals("SP", r.items().get(0).state());
    ...
    void clampsSizeToMax100() {
        ...
        when(customerRepository.findPage(eq(tenantId), captor.capture(), any(), any(), any())).thenReturn(new PageImpl<>(List.of()));
    ...
    @Test
    void normalizesStatusFilterToLowercaseDatabaseValue() {
        ArgumentCaptor<String> statusCaptor = ArgumentCaptor.forClass(String.class);
        when(customerRepository.findPage(eq(tenantId), any(), statusCaptor.capture(), any(), any()))
            .thenReturn(new PageImpl<>(List.of()));
        useCase.execute(new ListCustomerQuery(tenantId, 0, 20, "INACTIVE", null, null));
        assertEquals("inactive", statusCaptor.getValue());
    }

    @Test
    void doesNotFilterByStatusWhenNoneIsProvided() {
        when(customerRepository.findPage(eq(tenantId), any(), isNull(), any(), any()))
            .thenReturn(new PageImpl<>(List.of()));
        useCase.execute(new ListCustomerQuery(tenantId, 0, 20));
        verify(customerRepository).findPage(eq(tenantId), any(), isNull(), any(), any());
    }

    @Test
    void trimsCpfAndFullNameFiltersBeforePassingToRepository() {
        ArgumentCaptor<String> cpfCaptor = ArgumentCaptor.forClass(String.class);
        ArgumentCaptor<String> nameCaptor = ArgumentCaptor.forClass(String.class);
        when(customerRepository.findPage(eq(tenantId), any(), any(), cpfCaptor.capture(), nameCaptor.capture()))
            .thenReturn(new PageImpl<>(List.of()));
        useCase.execute(new ListCustomerQuery(tenantId, 0, 20, null, "  11144477735  ", "  João  "));
        assertEquals("11144477735", cpfCaptor.getValue());
        assertEquals("João", nameCaptor.getValue());
    }

    @Test
    void convertsBlankCpfAndFullNameFiltersToNull() {
        when(customerRepository.findPage(eq(tenantId), any(), any(), isNull(), isNull()))
            .thenReturn(new PageImpl<>(List.of()));
        useCase.execute(new ListCustomerQuery(tenantId, 0, 20, null, "   ", "   "));
        verify(customerRepository).findPage(eq(tenantId), any(), any(), isNull(), isNull());
    }
}
